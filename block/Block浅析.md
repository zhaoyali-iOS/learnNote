...
* Block截获对象型自动变量时，在Block里可以修改对象里面的内容，对象地址不能修改；默认是强引用对象(引用计数+1)，Block释放时捕获对象引用计数-1.
* Block使用对象的实例变量时，捕获的是实例变量所属的对象而不是实例变量本身。
* Block截获值传递型变量时，Block中可以修改后不会同步到block外部，除非使用__block修饰符。
* Block不支持C数组变量的截获。
...
```
小结：
* `__NSGlobalBlock__`：表达式中没有捕获任何自动变量，或者只有全局变量、静态(全局/局部)变量。
* `__NSStackBlock__`:Block创建时的类型，存储在栈上，超出作用域就会释放。
* `__NSMallocBlock__`：分配在堆上，一般是stack类型copy过去的。
* `=`：stack类型：把Block拷贝到堆上变为malloc类型；malloc类型：Block的引用计数+1；global类型：空操作
